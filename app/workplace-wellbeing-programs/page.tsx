    ...
    <>
      ...
      <section id="program-shape" className="bg-white py-20 lg:py-24">
        <ProductionContainer>
          ...
          <h2 className="mt-5 max-w-5xl text-4xl font-bold tracking-[-0.018em] text-[var(--cya-teal-dark)] lg:text-5xl">Start with the workplace, then build the rhythm around it</h2>
          <p className="mt-6 max-w-4xl text-lg leading-8 text-[var(--cya-body)]">A useful program is not assembled from a fixed package. We start with how work happens, shape the right mix of experiences and create a rhythm people can realistically use.</p>
          <div className="mt-12 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 md:grid md:grid-cols-4 md:gap-0 md:overflow-visible md:pb-0">
            {programSteps.map(([number,title,body]) => <article key={number} className="min-w-[82%] snap-start border-y border-r border-[var(--cya-divider)] bg-[var(--cya-surface-page)] px-6 py-8 first:border-l sm:min-w-[60%] md:min-w-0"><p className="text-xs font-semibold text-[var(--cya-ochre-ink)]">{number}</p><h3 className="mt-5 text-xl font-semibold leading-7 text-[var(--cya-teal-dark)]">{title}</h3><p className="mt-3 leading-7 text-[var(--cya-body)]">{body}</p></article>)}
          </div>
        </ProductionContainer>
      </section>
      ...
      <section className="bg-[var(--cya-surface-base)] py-20 lg:py-24">
        <ProductionContainer className="grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-stretch lg:gap-16">
          <figure className="relative min-h-[26rem] overflow-hidden bg-white lg:min-h-[34rem]"><Image src="/images/selected/cya-movement-group-industrial.webp" alt="Employees take part in a facilitated wellbeing session in an industrial workplace." fill sizes="(min-width: 1280px) 700px, (min-width: 1024px) 54vw, 100vw" className="object-cover" /></figure>
          <div className="flex flex-col justify-center"><ProductionKicker>Different workplaces, different rhythms</ProductionKicker><h2 className="mt-5 max-w-3xl text-4xl font-bold tracking-[-0.018em] text-[var(--cya-teal-dark)] lg:text-5xl">The program changes with the workplace</h2><p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--cya-body)]">The same wellbeing idea can need a different delivery approach depending on the environment, workforce and way the day is structured.</p><div className="mt-8 border-y border-[var(--cya-divider)]">{workplaceRhythms.map(([title,body],index) => <article key={title} className="grid grid-cols-[2.5rem_1fr] gap-4 border-b border-[var(--cya-divider)] py-4 last:border-b-0"><p className="pt-1 text-xs font-semibold text-[var(--cya-ochre-ink)]">{String(index+1).padStart(2,"0")}</p><div><h3 className="text-[1.05rem] font-semibold leading-6 text-[var(--cya-teal-dark)]">{title}</h3><p className="mt-1.5 max-w-lg leading-6 text-[var(--cya-body)]">{body}</p></div></article>)}</div></div>
        </ProductionContainer>
      </section>

      <section className="bg-[var(--cya-surface-page)] py-20 lg:py-24"><ProductionContainer className="grid gap-12 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-center lg:gap-16"><div><ProductionKicker>Wellbeing Studio by Corporate Yoga Australia</ProductionKicker><h2 className="mt-5 max-w-3xl text-4xl font-bold tracking-[-0.018em] text-[var(--cya-teal-dark)] lg:text-5xl">Support does not have to stop when the live session ends</h2><p className="mt-6 max-w-3xl text-lg leading-8 text-[var(--cya-body)]">For ongoing programs, Wellbeing Studio gives participants practical ways to return to wellbeing between scheduled experiences. It extends the human-led program rather than replacing it.</p><div className="mt-8"><ProductionAction href="/online-wellbeing" style="secondary">Explore online wellbeing</ProductionAction></div></div><figure className="relative aspect-[2048/970] overflow-hidden border border-[var(--cya-divider)] bg-white"><Image src="/images/selected/wellbeing-studio-placeholder.png" alt="Wellbeing Studio interface showing practical wellbeing support for different moments in the working day." fill sizes="(min-width: 1024px) 54vw, 100vw" className="object-contain" /></figure></ProductionContainer></section>

      <section className="bg-white py-20 lg:py-24"><ProductionContainer><div className="max-w-4xl"><ProductionKicker>Program experience in practice</ProductionKicker><h2 className="mt-5 text-4xl font-bold tracking-[-0.018em] text-[var(--cya-teal-dark)] lg:text-5xl">One relationship. Different needs over time</h2><p className="mt-6 text-lg leading-8 text-[var(--cya-body)]">Our work with Cromwell Property Group shows how an ongoing program can connect different experiences, locations and periods of support without relying on one fixed format.</p></div><div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(22rem,0.9fr)] lg:items-stretch lg:gap-12"><figure className="relative min-h-[28rem] overflow-hidden bg-[var(--cya-surface-page)]"><Image src="/images/selected/cya-workplace-wellbeing-lunch-learn-team.webp" alt="Employees take part in a facilitated workplace wellbeing experience." fill sizes="(min-width: 1280px) 720px, (min-width: 1024px) 58vw, 100vw" className="object-cover" /></figure><div className="bg-[var(--cya-surface-warm)] px-7 py-8 sm:px-9 lg:px-10 lg:py-10"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--cya-ochre-ink)]">Cromwell Property Group</p><div className="mt-6 border-y border-[var(--cya-mid-neutral)]/50">{["Recurring workplace yoga and Pilates","10- and 20-week programs","Workplace activations and Lunch & Learns","Digital wellbeing support","Delivery across Queensland, New South Wales and Victoria"].map((item,index)=><div key={item} className="grid grid-cols-[2.5rem_1fr] gap-3 border-b border-[var(--cya-mid-neutral)]/50 py-4 last:border-b-0"><span className="text-xs font-semibold text-[var(--cya-ochre-ink)]">{String(index+1).padStart(2,"0")}</span><p className="font-semibold leading-6 text-[var(--cya-teal-dark)]">{item}</p></div>)}</div><p className="mt-6 text-sm leading-6 text-[var(--cya-body)]">Program details are included only where facts and permissions have been confirmed.</p><div className="mt-7"><ProductionAction href="/case-studies" style="secondary">View case studies</ProductionAction></div></div></div></ProductionContainer></section>

      <section className="bg-[var(--cya-surface-base)] py-20 lg:py-24"><ProductionContainer><ProductionKicker>Clear delivery and review</ProductionKicker><h2 className="mt-5 max-w-4xl text-4xl font-bold tracking-[-0.018em] text-[var(--cya-teal-dark)] lg:text-5xl">Enough structure to make the program easy to run</h2><p className="mt-6 max-w-4xl text-lg leading-8 text-[var(--cya-body)]">We agree the practical details before launch, keep participation clear and use proportionate review to understand what should continue or change.</p><div className="mt-10 grid border-y border-[var(--cya-divider)] md:grid-cols-4">{[["01","Scope","Who, where, what and when."],["02","Participation","Clear information and voluntary access."],["03","Delivery","Appropriate facilitators and formats."],["04","Review","Aggregate participation and useful feedback."]].map(([number,title,body])=><article key={number} className="border-b border-[var(--cya-divider)] px-6 py-7 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0"><p className="text-xs font-semibold text-[var(--cya-ochre-ink)]">{number}</p><h3 className="mt-4 text-xl font-semibold text-[var(--cya-teal-dark)]">{title}</h3><p className="mt-2 leading-7 text-[var(--cya-body)]">{body}</p></article>)}</div></ProductionContainer></section>
