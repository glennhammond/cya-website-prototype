...
export function AnalyticsConsentManager({ enabled }: { enabled: boolean }) {
  ...
  useEffect(() => {
    const openPreferences = () => {
      if (enabled) setPreferencesRequested(true);
    };
  ...
  const showPanel = enabled && (preferencesRequested || consent === null);
      ...
      {showPanel ? (
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--cya-body)]">
            See the <Link href="/privacy-policy" className="font-semibold underline underline-offset-4">Privacy Policy</Link> for how we handle website information.
          </p>
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <button
              type="button"
              onClick={() => choose("granted")}
              className="min-h-12 rounded-[4px] bg-[var(--cya-action-primary-bg)] px-6 text-sm font-bold text-[var(--cya-action-primary-text)] focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[var(--cya-teal-dark)]"
            >
              Allow analytics
            </button>
            <button
