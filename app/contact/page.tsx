...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ interest?: string }> }) {
  ...
  return <>
    ...
    <section className="bg-white py-10 sm:py-14 lg:py-16"><ProductionContainer className="grid min-w-0 gap-8 lg:grid-cols-[1.4fr_0.8fr] lg:items-center"><div className="min-w-0"><ProductionKicker>Plan with CYA</ProductionKicker><h1 className="mt-4 max-w-4xl text-[clamp(2.5rem,11vw,4.5rem)] font-bold leading-[1.02]">{contextualLabel ? `Tell us about ${contextualLabel.toLowerCase()}.` : "Tell us what needs to happen."}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-[var(--cya-body)]">A few useful details are enough for CYA to understand the context and respond personally.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><ProductionAction href="#planning-form">Start planning</ProductionAction><ProductionAction href="mailto:hello@corporateyoga.com.au" style="secondary">Contact directly</ProductionAction></div></div><aside className="hidden bg-[var(--cya-teal-dark)] p-8 text-white lg:block"><ProductionKicker light>Planning starts here</ProductionKicker><ul className="mt-6 space-y-4"><li>People</li><li>Workplace context</li><li>What needs to happen</li></ul></aside></ProductionContainer></section>
