...
assert.match(release, /privacyPolicyApproved\(\)[\s\S]*websiteTermsApproved\(\)[\s\S]*CYA_INDEXING_ENABLED/);
...
for (const route of [
  ...
  "app/cart/route.ts",
  "app/blog/category/[slug]/route.ts",
  "app/blog/tag/[slug]/route.ts",
]) {
...
assert.equal(routeDecisions.version, "2.1");
assert.equal(routeDecisions.retiredRoutes.length, 6);
assert.equal(routeDecisions.legacyEditorialRedirects.length, 6);
assert.equal(routeDecisions.retiredRouteFamilies.length, 2);
...
assert.equal(approvals.renderedServerQaPassed, true, "rendered production-server QA must be recorded");
assert.equal(approvals.migrationHttpQualified, true, "local migration HTTP qualification must be recorded");
