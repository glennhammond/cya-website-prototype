...
export default function InsightsPage() {
  ...
  return (
    <>
      ...
      <section className="bg-white py-20 lg:py-24"><ProductionContainer><h2 className="text-4xl font-bold tracking-[-0.018em] text-[var(--cya-charcoal)] lg:text-5xl">Explore by topic</h2><div className="mt-10 flex snap-x snap-mandatory gap-0 overflow-x-auto border-y border-[var(--cya-divider)] pb-0 md:grid md:grid-cols-4 md:overflow-visible">{topics.map(([title,body,href], index) => <Link id={title.toLowerCase().replace(" ", "-")} key={title} href={href} className="group min-w-[78%] snap-start border-r border-[var(--cya-divider)] px-6 py-7 last:border-r-0 sm:min-w-[50%] md:min-w-0"><article><p className="text-xs font-semibold text-[var(--cya-ochre-ink)]">{String(index + 1).padStart(2, "0")}</p><h3 className="mt-4 text-2xl font-semibold text-[var(--cya-charcoal)] group-hover:text-[var(--cya-teal-dark)]">{title}</h3><p className="mt-3 leading-7 text-[var(--cya-body)]">{body}</p><span className="mt-5 inline-flex font-semibold text-[var(--cya-charcoal)]">Explore →</span></article></Link>)}</div><p className="mt-3 text-sm text-[var(--cya-body)] md:hidden">Swipe to explore →</p></ProductionContainer></section>
